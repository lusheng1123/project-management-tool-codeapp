...
export function ResourcesView() {
  ...
  return (
    <div>
      <div className="dashboard-header"><h2>👥 Resource Management</h2><button className="btn btn-primary" onClick={() => { const deptConfigs = DS.query('pm_config', { pm_type: 'department' }); const teamConfigs = DS.query('pm_config', { pm_type: 'team' }); const rsConfigs = DS.query('pm_config', { pm_type: 'resource_status' }); showModal({ title: 'New Resource', fields: getFields('pm_resource').filter(f => f.name !== 'pm_department' && f.name !== 'pm_team' && f.name !== 'pm_status'), extraContent: (<div><label>Department</label><select id="resDept" data-extra><option value="">None</option>{deptConfigs.map((c: any) => <option key={c.id} value={c.pm_name}>{c.pm_name}</option>)}</select><label>Team</label><select id="resTeam" data-extra><option value="">None</option>{teamConfigs.map((c: any) => <option key={c.id} value={c.pm_name}>{c.pm_name}</option>)}</select><label>Status</label><select id="resStatus" data-extra><option value="">None</option>{rsConfigs.map((c: any) => <option key={c.id} value={c.pm_name}>{c.pm_name}</option>)}</select></div>), onSave: (fd) => { fd.pm_department = (document.getElementById('resDept') as HTMLSelectElement)?.value || ''; fd.pm_team = (document.getElementById('resTeam') as HTMLSelectElement)?.value || ''; fd.pm_status = (document.getElementById('resStatus') as HTMLSelectElement)?.value || ''; DS.create('pm_resource', fd); reload(); showToast('Resource created!') } }) }}>+ New Resource</button></div>
      ...
      {!filtered.length ? <EmptyState msg="No resources found" /> : (
        ...
        <tbody>{filtered.map((r: any) => {
          ...
          return (<Fragment key={r.id}>
            <tr className={`data-row${isExp ? ' project-row-expanded' : ''} project-main-row`} onClick={() => toggle(r.id)} style={{ cursor: 'pointer' }}><td><strong>{r.pm_name}</strong></td><td>{r.pm_role}</td><td><span className={`badge ${badgeClass(r.pm_department)}`}>{r.pm_department}</span></td><td><span className={`badge ${badgeClass(r.pm_team)}`}>{r.pm_team || '—'}</span></td><td><span className={`badge ${badgeClass(r.pm_status)}`}>{r.pm_status}</span></td><td>{r.pm_cost ? '$'+(Number(r.pm_cost)).toLocaleString() : '—'}</td><td><span style={{ fontWeight: 700, color: capColor }}>{totalAlloc}%</span></td><td className="actions-cell" onClick={e => e.stopPropagation()}><button className="btn-sm btn-edit" onClick={() => { const rec = DS.getById('pm_resource', r.id); const deptConfigs = DS.query('pm_config', { pm_type: 'department' }); const teamConfigs = DS.query('pm_config', { pm_type: 'team' }); const rsConfigs = DS.query('pm_config', { pm_type: 'resource_status' }); showModal({ title: 'Edit Resource', fields: getFields('pm_resource').filter(f => f.name !== 'pm_department' && f.name !== 'pm_team' && f.name !== 'pm_status'), data: rec, extraContent: (<div><label>Department</label><select id="resDept" data-extra defaultValue={rec.pm_department || ''}><option value="">None</option>{deptConfigs.map((c: any) => <option key={c.id} value={c.pm_name}>{c.pm_name}</option>)}</select><label>Team</label><select id="resTeam" data-extra defaultValue={rec.pm_team || ''}><option value="">None</option>{teamConfigs.map((c: any) => <option key={c.id} value={c.pm_name}>{c.pm_name}</option>)}</select><label>Status</label><select id="resStatus" data-extra defaultValue={rec.pm_status || ''}><option value="">None</option>{rsConfigs.map((c: any) => <option key={c.id} value={c.pm_name}>{c.pm_name}</option>)}</select></div>), onSave: (fd) => { fd.pm_department = (document.getElementById('resDept') as HTMLSelectElement)?.value || ''; fd.pm_team = (document.getElementById('resTeam') as HTMLSelectElement)?.value || ''; fd.pm_status = (document.getElementById('resStatus') as HTMLSelectElement)?.value || ''; DS.update('pm_resource', r.id, fd); reload(); showToast('Resource updated!') }, onDelete: () => { DS.delete('pm_resource', r.id); reload(); showToast('Resource deleted!') } }) }}>✏️ Edit</button><button className="btn-sm btn-delete" onClick={() => { if (confirm('Delete this resource?')) { DS.delete('pm_resource', r.id); reload(); showToast('Resource deleted!') } }}>🗑️</button></td></tr>
